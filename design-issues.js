...
(function(){
  ...
  function collectIssue(){
    ...
    if(!ref||!title||!responsible)throw new Error("Issue reference, issue/clarification and responsible party are required.");
    const r={project:activeProject(),ref,category:$("diCategory").value,title,tower:$("diTower").value.trim(),floor:$("diFloor").value.trim(),responsible,priority:$("diPriority").value,status:$("diStatus").value,raised:$("diRaised").value,target:$("diTarget").value,impact:$("diImpact").value,action:$("diAction").value.trim(),remarks:$("diRemarks").value.trim()};
